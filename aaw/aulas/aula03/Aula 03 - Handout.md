...
1. Qual(is) problema(s) de design vocês identificam? Usa POST para consulta e retorna dados demais.

2. Seu redesenho (método + rota + status codes): GET /api/alunos vai dar 200 ok
...
1. Qual(is) problema(s) de design vocês identificam? Usa GET para excluir e retorna 200 mesmo se nao existir

2. Seu redesenho (método + rota + status codes):DELETE /api/alunos/7 vai dar 204 No Content. Se não existir: 404 Not Found.
...
1. Qual(is) problema(s) de design vocês identificam? retorna 200 e nao informa o ID criado, obrigando nova consulta

2. Seu redesenho (método + rota + status codes): POST /api/alunos → 201 Created + dados do aluno criado e Location
...
1. Qual(is) problema(s) de design vocês identificam? rota muito longa e exige vários IDs desnecessarios

2. Seu redesenho (método + rota + status codes): GET /api/disciplinas/12 → 200 OK
...
1. Qual(is) problema(s) de design vocês identificam? retorna 200 com HTML em vez de jSON e codigo de erro.

2. Seu redesenho (método + rota + status codes): GET /api/alunos/7/matriculas → 200 OK. Se o aluno não existir: 404 Not Found + JSON
...
1. Qual(is) problema(s) de design vocês identificam? Usa query string para alteração e  nao utiliza body

2. Seu redesenho (método + rota + status codes):PATCH /api/alunos/7/disciplinas/12 + body { "nota": 8.5 } → 200 OK.
...
1. A EscolaTech quer lançar mudanças na API sem quebrar o app mobile antigo, que não recebe atualização há 2 anos. Que decisão de design — que falta na API INTEIRA — resolve esse problema? Como ficariam as rotas?
 ...
 A solução é versionar a API, para manter a versão antiga funcionando.
...
Assim, o app antigo continua usando v1, enquanto novos aplicativos usam v2.